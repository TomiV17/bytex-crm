...
// Obtener usuarios
$result = $conexion->query("SELECT * FROM usuarios $where_sql ORDER BY fecha_creacion DESC");
?>
...
<main>
    <h1>Usuarios</h1>
    <a href="controllers/crear_usuario.php" class="button">Agregar Usuario</a>
    <section>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <?php 
                            $emp = $conexion->query("SELECT nombre FROM empresas WHERE id=".(int)$row['empresa_id'])->fetch_assoc();
                            echo $emp['nombre'] ?? 'Sin empresa';
                        ?>
                    </td>
                    <?php endif; ?>
                    <td><?= $row['fecha_creacion'] ?></td>
                    <td>
                        <a href="controllers/editar_usuario.php?id=<?= $row['id'] ?>">Editar</a> |
                        <a href="controllers/eliminar_usuario.php?id=<?= $row['id'] ?>" onclick="return confirm('¿Eliminar usuario?')">Eliminar</a>
                    </td>
                </tr>
                <?php endwhile; ?>
                <?php if($result->num_rows==0): ?>
                <tr>
                    <td colspan="<?= $is_global ? 7 : 6 ?>" style="text-align:center;">No se encontraron usuarios</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
